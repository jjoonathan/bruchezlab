...
for arg in sys.argv[1:]:
	if object_sel_regex==1.23:
		object_sel_regex = re.compile(arg)
	elif arg=="-obj":
		object_sel_regex = 1.23
	elif infile==None:
		infile = arg
if infile==None or not os.path.exists(infile):
	print "Usage: %s <in-name>.pdb"%sys.argv[0]
	print "   Option: -obj obj_sel_regex (for re.match)"
	print "   Output: <in-name>.<chain1>.fasTSV"
	print "   Output: <in-name>.<chain2>.fasTSV"
	print "   Output: ..."
...
cmd.load(infile)
chain2file = {}
print "PROGRAM IS INCOMPLETE. It loads the infile, but everything after that is todo."
for obj_name in cmd.get_names("objects"):
	if object_sel_regex not in (None, 1.23):
		if not object_sel_regex.match(obj_name):
			continue
	of.write(">%s\n"%(obj_name,))
	of2.write(">%s\n"%(obj_name,))
